...
# Library for PiMotor Shield V2. NOTE: My modifications start on line 183.
...
class Stepper:
    ...
    '''
    def forward(self, delay, steps): # Sequence of high/low signals changed
                                      # to be compatible with our steppers.
        for i in range(steps):       # These 4 mini-steps rotate a stepper
            self.setStep(1, 0, 1, 0)  # one full 7.5-degree step. We split this
            time.sleep(delay)         # into 2 halves to double the steps for
            self.setStep(0, 1, 1, 0)  # finer movement.
            time.sleep(delay)
    ...
    def forward1(self, delay):   # This is the first half of forward(), which
                                  # can be alternated with forward2() to
                                  # allow continuous movement as usual.
                self.setStep(1, 0, 1, 0)
                time.sleep(delay)
                self.setStep(0, 1, 1, 0)
                time.sleep(delay)

    def forward2(self, delay):  # This is the second half of forward().
    ...
    ''' Rotate Stepper motor in backward direction
        ...
        steps = Number of Steps
    '''
    def backward(self, delay, steps):   # We split this backward function
                                        # similarly.
    ...
    def backward1(self, delay):   # First half of backward().
    ...
    def backward2(self, delay):     # Second half of backward().
